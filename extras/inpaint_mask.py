...
def generate_mask_from_image(
...
) -> tuple[np.ndarray | None, int | None, int | None, int | None]:
    ...
    if mask_model != "sam" or sam_options is None:
        result = remove(
            image, session=new_session(mask_model, **extras), only_mask=True, **extras
        )
    ...
    if dino_detection_count > 0:
        ...
        if sam_options.dino_debug:
            ...
            debug_dino_image = Image.new(
                "RGB", (image.shape[1], image.shape[0]), color="black"
            )
            draw = ImageDraw.Draw(debug_dino_image)
            for box in boxes.numpy():
                draw.rectangle(box.tolist(), fill="white")
            return (
                np.array(debug_dino_image),
                dino_detection_count,
                sam_detection_count,
                sam_detection_on_mask_count,
            )
    ...
    mask_image = (
        np.dstack((final_mask_tensor, final_mask_tensor, final_mask_tensor)) * 255
    )
