...
def load_parameter_button_click(
    raw_metadata: dict | str, is_generating: bool, inpaint_mode: str
):
    ...
    get_inpaint_engine_version(
        "inpaint_engine_version",
        "Inpaint Engine Version",
...
def get_steps(
    ...
    try:
        ...
        performance_candidates = [
            ...
            if key.casefold() == performance_name and Steps[key] == h
        ]
...
def get_inpaint_engine_version(
    key: str,
    fallback: str | None,
    source_dict: dict,
    results: list,
    ...
    for settings_key, meta_key in modules.config.possible_preset_keys.items():
        if settings_key == "default_loras":
            ...
            for index, lora in enumerate(
                loras[: modules.config.default_max_lora_number]
            ):
