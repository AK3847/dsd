...
for image_count in range(default_controlnet_image_count):
    ...
    default_end, default_weight = modules.flags.default_parameters[
        default_ip_types[image_count]
    ]
...
example_enhance_detection_prompts = get_config_item_or_set_default(
    ...
    default_value=["face", "eye", "mouth", "hair", "hand", "body"],
    validator=lambda x: isinstance(x, list) and all(isinstance(v, str) for v in x),
    expected_type=list,
)
...
default_inpaint_mask_sam_model = get_config_item_or_set_default(
    ...
    in [y[1] for y in modules.flags.inpaint_mask_sam_model if y[1] == x],
    expected_type=str,
)

config_dict["default_loras"] = default_loras = default_loras[
    :default_max_lora_number
] + [[False, "None", 1.0] for _ in range(default_max_lora_number - len(default_loras))]
